...
- [`fixtures/`](fixtures/README.md): the maps and operators everything runs
  on; [`plinopt/`](fixtures/plinopt/README.md) is thirteen of PLinOpt's own
  files under CeCILL-B, bytes untouched.
- [`benchmark_tensors/`](benchmark_tensors/README.md): the tensors the
  literature argues about, where each search stops on them, and the one
  owner of what is known about each.
- [`reproduce/`](reproduce/README.md): measure.py, which regenerates every
