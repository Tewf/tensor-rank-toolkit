...
# The four the descent is measured on, and deliberately only those. `evidence/fixtures/`
...
# (`evidence/fixtures/published-targets.md`) have no such row to re-derive.
#
...
# `methods/bilinear_rank/flip_graph/results.json` publishes a crossing at exactly these flags, and the
...
CARRIED = (
    ...
    ("methods/matrix_sparsification/results.json", "3x3x3_23_Grey-221",
     ...
     "not among the thirteen `evidence/fixtures/plinopt/` ships, so `sparsify-operator` has nothing to run on, "
     ...
     "table to the two it prints is `evidence/reproduce/front_page.py`."),
)
...
def descent_of(build, name, repeats=REPEATS):
    ...
    command = [str(build / "methods/bilinear_rank/greedy_heuristic" / "minimise-rank"),
               str(ROOT / "evidence/fixtures" / f"{name}.tensor"), "--steps", "3"]
...
def sparsification_of(build, name, repeats=REPEATS):
    ...
    """
    binary = str(build / "methods/matrix_sparsification" / "sparsify-operator")
    operator = str(ROOT / "evidence/fixtures" / f"{name}.matrix")
...
def tree_search(build, fixture, target=None, expect=(0,), repeats=REPEATS, symmetry=None):
    ...
    command = [str(build / "methods/bilinear_rank/exhaustive" / "decide-rank"),
               str(ROOT / "evidence/fixtures" / f"{fixture}.tensor")]
...
def heuristic_of(build, fixture, steps, repeats=REPEATS):
    ...
    command = [str(build / "methods/bilinear_rank/greedy_heuristic" / "minimise-rank"),
               str(ROOT / "evidence/fixtures" / f"{fixture}.tensor"), "--steps", str(steps)]
...
def satisfiability_of(build, question, committed, repeats=REPEATS, slow=False):
    ...
    """
    name = question["name"]
    tensor = str(ROOT / "evidence/fixtures" / f"{name}.tensor")
    solver = str(build / "methods/satisfiability" / "decide-rank-by-sat")
    row = dict(committed)
...
def plateau_of(build, question, repeats=REPEATS):
    """One plateau crossing, as the row `methods/bilinear_rank/flip_graph/results.json` publishes.
    ...
    """
    command = [str(build / "methods/bilinear_rank/greedy_heuristic" / "minimise-rank"),
               str(ROOT / "evidence/fixtures" / f"{question['tensor']}.tensor"), "--steps", "3",
...
def walk_of(build, question, repeats=REPEATS):
    """One flip walk, as the row `methods/bilinear_rank/flip_graph/results.json` publishes.
    ...
    """
    command = [str(build / "methods/bilinear_rank/flip_graph" / "walk-scheme"),
               str(ROOT / "evidence/fixtures" / f"{question['tensor']}.tensor"),
