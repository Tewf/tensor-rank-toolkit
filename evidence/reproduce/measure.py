...
reader who disagrees with a number can rerun exactly what made it.
    ...
    evidence/reproduce/measure.py --build build                 # rewrite the results files
    evidence/reproduce/measure.py --build build --check         # counts only, exit 1 on drift
    evidence/reproduce/measure.py --build build --check --slow  # and the 4.1 minute exhaustion
    evidence/reproduce/measure.py --build build --counts        # rewrite counts, keep every timing
...
# script's own directory first on the path, so `evidence/reproduce/measure.py` runs from
...
def main():
    ...
    sparsification = ROOT / "methods/matrix_sparsification" / "results.json"
    satisfiability = ROOT / "methods/satisfiability" / "results.json"
