...
| `--route auto\|exhaustive\|sat\|canonical` | `auto`, which takes the solver past **20 000** pool matrices | Measured. On `matmul_2x2x2` the solver is 0.54 s and 6.0 MB against the pool's 1.01 s and 11.9 MB; on `<4,4,4>` the pool is refused outright at 8.2 TiB against a 2.0 GiB budget while the solver starts. On `gf16_multiplication` the pool route returns 12 as an upper bound in 1 m 31 s where the solver returns the rank, 9, in 2 m 12 s, so it finishes first only by giving up first. [`../canonical_factorisation/routes.md`](../canonical_factorisation/routes.md). `canonical` is McKay augmentation and never the default: **53x fewer nodes** for 8.1x the wall clock on the `matmul_2x2x2` sweep. Swept level by level at five shapes it loses at every one but `<3,3,3>` at 10, where it wins 1.33x with **no node saving at all**; what decides it is seven to fourteen pool scans a node, 98% of the cost at `<2,3,3>`, and not the group. [`../canonical_factorisation/canonical-augmentation.md`](../canonical_factorisation/canonical-augmentation.md), and when it would pay: [`../oracle_guided_search/when-canonical-pays/`](../oracle_guided_search/when-canonical-pays/README.md). |
