...
| `--route auto\|exhaustive\|sat` | `auto`, which takes the solver past **20 000** pool matrices | Measured. On `matmul_2x2x2` the solver is 0.54 s and 6.0 MB against the pool's 1.01 s and 11.9 MB; on `<4,4,4>` the pool is refused outright at 8.2 TiB against a 2.0 GiB budget while the solver starts. On `gf16_multiplication` the pool route returns 12 as an upper bound in 9 m 52 s where the solver returns the rank, 9, in 2 m 12 s. [`../canonical_factorisation/routes.md`](../canonical_factorisation/routes.md). |
