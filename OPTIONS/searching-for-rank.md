...
| `--bottom-up` | off | **Measured, and it never wins.** It selects `fewest_products_from_scratch`, which absorbs the map's slices in rank order and which its own header calls "the most expensive approach". Against the default route on the six fixtures where both finish: same answer on all six, equal nodes on two, and worse on four, the widest being `gf8_multiplication` at **506 nodes against 329**. Never fewer, on anything. Its stated case is that its answer is not conditioned on a heuristic's output, and that is already what `--anchor map` gives, which is the default. Retired to `rejected-experiments` on that basis; the numbers are here so the rejection is not a whim. |
