...
| `--solver <name>` | none; `sat_solver_order` decides | **Measured, and the order is the interesting half.** Native XOR clauses are worth nothing on these instances, **1.559 s against 1.563 s** on the same question, while kissat's raw strength is worth several times: **0.339 s** on that question, **34.3 s against 167.9 s** on the next (`../satisfiability/choices/`). Read the ratio with care: the same preference is written up as "five times" in `choices/` and as "**126x**, 0.2866 s against 36.19 s" on GF(16) at 9 products in `results.json`. Those are two questions, not two measurements of one. That 126 was 134 while the kissat end was the 2026-08-16 figure, and the two are one claim: a solver second is another program's, and 6% apart is inside the noise floor. |
| `--seed N` | `local_search_seed`, `1` | Argument: it reaches only a solver that can only find (`--solver yalsat`, `probSAT` or `multilinear-sat`, by name or by path), and such a solver is a Las Vegas algorithm, right whenever it answers and random in how long it takes. One seed is one draw, so a published number carries its seed and a cost is a distribution: at least five seeds, the median, and how many finished within the cap, in [`../satisfiability/las-vegas/`](../satisfiability/las-vegas/README.md). Fixed rather than clocked so that a run reproduces. A no from such a solver is the third answer, never a bound, and `--proof` is refused. |
...
| `--emit-xnf <path>` | none | Nothing to measure: the same file with each parity as one `x` line, the XNF that xnfsat, `cnf2xnf` and cryptominisat read, under the name of its format. Refused together with `--plain-cnf`, which asks for the other file. Why the two files are not interchangeable for a local search, with the reference: [`../satisfiability/method/gf2-as-cnf.md`](../satisfiability/method/gf2-as-cnf.md). |
