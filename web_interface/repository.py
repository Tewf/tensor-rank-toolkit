...
The command is rendered by `evidence/reproduce/questions.py`, which already had to solve
...
# `evidence/reproduce/` is imported flat, the way its own two files import each other, so
# no package, no installer and no `__init__.py` anywhere.
sys.path.insert(0, str(ROOT / "evidence/reproduce"))
...
def fixtures():
    ...
    """
    tensors = sorted(path.name for path in (ROOT / "evidence/fixtures").glob("*.tensor"))
    operators = sorted(path.name for path in (ROOT / "evidence/fixtures").glob("*.matrix"))
    plinopt = sorted("plinopt/" + path.name
                     for path in (ROOT / "evidence/fixtures" / "plinopt").glob("*.sms"))
    return {"tensor": tensors, "operator": operators + plinopt}


def fixture_path(name):
    """One fixture's path, refused unless it is inside `evidence/fixtures/`.
    ...
    """
    directory = (ROOT / "evidence/fixtures").resolve()
