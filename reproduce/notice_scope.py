...
Two checks, both exact:
  ...
  - the number NOTICE writes for fixtures/plinopt/'s matrices is the number of
...
def failures():
    ...
    counted = re.search(r"fixtures/plinopt/ is (\w+) `\.sms` matrices", notice)
    if counted is None:
        yield "NOTICE no longer counts fixtures/plinopt/'s matrices at all"
        return
    word = counted.group(1)
    if word not in NUMBER_WORDS:
        yield f"NOTICE counts fixtures/plinopt/ as '{word}', not a number word"
        return
    on_disk = len(list((ROOT / "fixtures" / "plinopt").glob("*.sms")))
    if NUMBER_WORDS[word] != on_disk:
        yield (f"NOTICE says {word} ({NUMBER_WORDS[word]}) `.sms` matrices; "
               f"fixtures/plinopt/ holds {on_disk}")
