...
\subsection{Four names, and two unrelated hardness proofs}\label{subsec:sparsehard}

The inner question, the lightest nonzero vector of a subspace, is one object the
literature names four ways: the \emph{minimum distance} of a linear code over
$\GF(q)$; the \emph{sparsest vector in a subspace} over $\mathbb{R}$ or
$\mathbb{Q}$ \cite{qu2020}; the \emph{spark} of a matrix \cite{tillmann2014}; and
...
\subsection{The exact oracle, and why the greedy is minimal}\label{subsec:sparsegreedy}
...
\subsection{Against the reference implementation}\label{subsec:plinopt}

PLinOpt \cite{plinopt} is the reference implementation in this problem area. It
reaches sparsity by a different route, sparse QLUP elimination and a coefficient
search bounded to four rows of support with a set of eleven coefficients, and it
is a heuristic where Theorem~\ref{thm:sparseexact} is a minimum.
...
\subsection{Fewer nonzeros is not fewer additions}\label{subsec:costmodels}

Nonzeros are a proxy for additions and the proxy is imperfect, so a count means
nothing until the model it is counted in is named. There are three, and they are
not comparable.
...
is a proxy that Section~\ref{subsec:costmodels} measures going the wrong way on
