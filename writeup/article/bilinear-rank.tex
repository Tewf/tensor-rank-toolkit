...
$128$, and we separate the three incomparable cost models in which such a count
can be read, since on one operator in nine the fewer nonzeros cost additions. We also record three
...
be. On the three operators of Table~\ref{tab:grey221}, where $N=23$ and $d=9$,
...
\emph{On a published scheme.} Table~\ref{tab:grey221} gives the three operators
of a published rank-23 $\langle 3,3,3\rangle$ scheme as they are published and at
the minimum Theorem~\ref{thm:sparseexact} returns, which is $221$ nonzeros taken
to $128$. Nothing in the second column is a best-effort figure: it is the least
any invertible $\Phi$ can leave, and the method reaches it over $\GF(2)$ and
$\GF(3)$ as well as over $\mathbb{Q}$, which is what the operators the descent of
Section~\ref{sec:descent} emits require. Every entry of the three minimising
matrices is $0$, $1$ or $-1$, so the count hides no multiplication: an entry
outside $\{0,\pm 1\}$ would cost one on top of its addition, and there is none.
That is a property of the bases returned here rather than of
Theorem~\ref{thm:sparseexact}, which minimises zeros and breaks ties by the order
it walks supports in.

\begin{table}[ht]
\centering
\begin{tabular}{llrrr}
\toprule
operator & shape & as given & minimum & seconds \\
\midrule
$L$   & $23\times 9$ & $69$  & $\mathbf{43}$  & $0.338$ \\
$R$   & $23\times 9$ & $66$  & $\mathbf{42}$  & $0.327$ \\
$P$   & $9\times 23$ & $86$  & $\mathbf{43}$  & $0.434$ \\
\midrule
total &              & $221$ & $\mathbf{128}$ & \\
\bottomrule
\end{tabular}
\vspace{0.5em}
...
better was found.}\label{tab:grey221}
\end{table}

Three things qualify that table, and each is the kind of qualification a count
invites.

\emph{Both columns are carried and neither is re-derived here.} Those three
operators are not among the fixtures shipped alongside this work, so no run of it
produces either number: both were measured once, on one machine, and the results
file that holds them names itself as carried on every pass. A carried number
reported as a fresh one is the failure that machinery exists to prevent.

\emph{$P$ is sparsified transposed.} It is $9\times 23$ and computes $9$ outputs
from $23$ products, so the admissible change of basis acts on its rows and the
object is its row space. Sparsified in its stored orientation it would be the
$23$ products that were recombined, which no change of basis on the outputs may
do, and the count returned would be a count about a different object.
...
four fixtures, by one product each time, at one to two orders of magnitude the
cost of the first two steps combined. The measured ratios span $36$ to $189$ and
are deliberately not quoted to two digits: the first two steps finish in
milliseconds, where the fixed cost of starting a process is a large share of what
was timed, and a repeat run moved one fixture's ratio from $189$ to $85$ with
nothing changed in the code. Effort spent accelerating step~3 is spent on the
