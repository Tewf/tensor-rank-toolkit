...
- [`already-published.md`](already-published.md): the record and the published
  methods, which are not ours to claim.
- [`where-we-sit.md`](where-we-sit.md): the one open question the rank strand
  has a contribution available on, and the first measurement against it.
- [`the-sparsification-strand.md`](the-sparsification-strand.md): the other
  strand, where the finding is an absence, and how narrowly to read it.
- [`what-this-changes.md`](what-this-changes.md): the next flip graph run this
  position implies.
