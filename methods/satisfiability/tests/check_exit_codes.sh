...
#
# It lives under methods/satisfiability/ because that is where the convention was first
...
command=$binaries/methods/matrix_sparsification/sparsify-operator
echo "sparsify-operator"
expect 2
expect 0 "$fixtures/strassen_u.matrix"
expect 5 "$fixtures/no_such_file.matrix"

command=$binaries/methods/curve_bounds/curve-bounds
echo "curve-bounds"
expect 2
expect 2 --nonsense
expect 0 --table
...
command=$binaries/methods/integer_programme/list-solvers
echo "list-solvers, retired"
expect 2
expect 2 --help

command=$binaries/methods/pencil_rank/decide-rank-by-pencil
...
command=$binaries/methods/canonical_factorisation/factor-over-canonical-basis
