...
3. **Still open: give what is left of the pool pass the packed GF(2) leaf.**
   ...
   general path, one reduction at a time, where `methods/bilinear_rank/exhaustive/gf2_leaf.h`
   ...
   privately, which is a change in `methods/bilinear_rank/exhaustive/` rather than here.
