...
`core/linear_algebra/rank_lower_bound.h` is where the maximum is taken, so a fourth
...
- Two test expectations, on `f2_5x5` and nowhere else.
  ...
  `core/linear_algebra/tests/test_rank_sum.cpp` asserted `rank_lower_bound` is
...
[`lower-the-bound`](../bilinear_rank/branch_and_bound/README.md) exhibited one; the sentence
