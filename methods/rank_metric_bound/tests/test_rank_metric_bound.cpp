...
/// `methods/pencil_rank/tests/test_projection_lower_bound.cpp` states: a test pinning a
...
struct Fixture {
    ...
    /// they sit in `evidence/fixtures/published-targets.md` as targets and this column
    ...
    /// `core/linear_algebra/rank_lower_bound.h` were made. Pinned here because it is
...
///
/// The same construction as `core/linear_algebra/tests/test_flattening.cpp`'s
