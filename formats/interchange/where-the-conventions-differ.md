...
it. The other side is LinBox 1.7.0 as packaged on Ubuntu 24.04, which is what PLinOpt
...
| type letter | ignored. 15 of the 153 matrices are `M`, 9 of those with negative entries and no modulus | `R` for rationals, `M` for GF(p), on writing; all eight letters accepted on reading | yes, and neither side depends on it |
| triples per line | **one.** The value is read by Givaro's line-greedy rational operator, so `1 1 5 3 2 7` is a single entry of 5327, silently | refused, naming the value | **no**, and ours refuses what theirs misreads |
| triple across lines | accepted | accepted | yes |
| entry | whatever ring the stream was built over: `sms2pretty` uses `Q[X]` and three of those files carry `2X`, `3*X` | `Q`, or `GF(p)` with a field; a polynomial is refused by name | **no**, ours is narrower |
...
All 153 `.sms` in PLinOpt's `data/`, read here and written straight back, then both
...
- `4o4o4_F32_Montgomery_P.sms` is the 150th: its first matrix comes back
  identical and the other three are dropped, as those checkers drop them.
