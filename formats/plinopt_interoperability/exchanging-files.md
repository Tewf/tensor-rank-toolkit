...
[`ours-through-his-checker.md`](ours-through-his-checker.md) and
[`his-file-through-ours.md`](his-file-through-ours.md).

## What to install

Nothing, to read and write his files: `.sms` is handled here and Givaro is
already this repository's only dependency. To check our output with *his*
...
flags. Its `make check` runs his own examples and is the fastest way to tell
...
`SUCCESS` from his checker is an independent confirmation: he derives the
...
[`what-both-sides-do.md`](what-both-sides-do.md). The four that will bite you:
...
- **A rational algorithm has moduli it does not survive.** `-q 2` on an
  algorithm with halves is refused here, naming the entry; his checkers report
  the same triple as not an algorithm, which is true and less specific.
- **Three of his 153 matrices are not rationals at all**: the `-X` family from
  the accuracy paper carries polynomials in an indeterminate. Refused by name.

Twelve of his files are vendored so this is exercised by the test suite and not
only by hand: [`../../fixtures/plinopt/`](../../fixtures/plinopt/README.md), and
