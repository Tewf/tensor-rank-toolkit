...
- **The repository is regrouped, and the tree now says what the code
  says.** Five groups: core/ (the exact arithmetic and the formats),
  methods/ (one folder per question), infrastructure/ (cli, run_limits,
  testing, gpu_leaf, tools), evidence/ (fixtures, benchmark_tensors,
  reproduce) and writeup/ (article, how-the-search-works, positioning,
  the-research-front). Inside methods/, bilinear_rank/ is the home of the
  one namespace its eight members always shared, with the vocabulary none
  of them owned at its root: the rank-one pool, the reflected Gray order,
  the (L,R,P) recovery with operators-to-tensor, and the aliases header.
  Four members took the field's own names: descent_search is
  greedy_heuristic, exhaustive_search is exhaustive, incumbent_search is
  branch_and_bound, oracle_guided_search is canonical_augmentation, each
  the term its own header already used. famous_tensors is
  benchmark_tensors and now solely owns the known-ranks record;
  satisfiability/search/ is bracket/, ending a near-homograph;
  card_failure lives in run_limits beside the device it reports on. Two
  commands renamed with their retirements recorded in OPTIONS:
  deflate-strictly is decide-rank-by-deflation, lower-the-bound is
  tighten-rank-bound. Every move went through git mv so history follows;
  results.json invocations were regenerated by measure.py --counts with
  every seconds field byte-identical; the licence list, the guards, the
  console catalogue and roughly 1100 relative links moved in step, and
  the whole battery ran green after every tier. One planned nesting was
  refused on evidence: integer_programme stays a methods/ sibling because
  the sparsifier's simplex route links it too, so the common ancestor is
  the methods level.
