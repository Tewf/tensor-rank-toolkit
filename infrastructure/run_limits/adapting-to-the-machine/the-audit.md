...
| **map construction** `methods/bilinear_rank/map_construction/` | builds `rows*columns` slices and writes them out `:98` | **NO**: one item, fixed work, output is a file | n/a by design: `make-tensor`'s flags are modes, not settings | ✖ `--matmul 2 100 100 100` asks for ~10¹² `int64_t` unguarded | ✖ |
