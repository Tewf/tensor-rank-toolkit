...
`methods/bilinear_rank/greedy_heuristic/minimum_weight_basis.cpp:251` (`span_element_ranks`), and
...
1. **It is the general-field path, and that field has none of the shape.**
   `infrastructure/gpu_leaf/README.md` already says it: *"Givaro carries every element as an
...
- `methods/satisfiability/tests/check_exit_codes.sh` uses `--max-memory 1` to assert that
