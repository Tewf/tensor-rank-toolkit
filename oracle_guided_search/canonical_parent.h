...
/// once a node rather than once a test.** It is `canon.canonical` of the parent's
/// pool content, which is the same set for every child of that parent, so
/// recomputing it per child was `c` copies of one answer. Holding it also turns the
/// first condition into an early exit: the parent is canonical exactly when no
/// candidate's name is strictly smaller, so the loop stops at the first one that
/// is, instead of naming every candidate to find a minimum it then compares against.
