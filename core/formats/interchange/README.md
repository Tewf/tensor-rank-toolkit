...
direction each way: **given a file published elsewhere, produce the file this
repository reads**, and given ours, produce one the publisher's tools can read
back.
...
[`methods/bilinear_rank/greedy_heuristic/commands/minimise_rank_main.cpp`](../../../methods/bilinear_rank/greedy_heuristic/commands/minimise_rank_main.cpp),
and read, [`methods/bilinear_rank/commands/operators_to_tensor_main.cpp`](../../../methods/bilinear_rank/commands/operators_to_tensor_main.cpp).

| | |
|---|---|
| [**`exchanging-files.md`**](exchanging-files.md) | **start here**: what to install, both directions in four lines, the command that checks our output with an external tool, and the four differences that cause failures in practice |
