...
- [`linear_algebra/`](linear_algebra/): matrices, spans in reduced
  row echelon form, and the search-free lower bounds, over GF(p) and over Q
  via Givaro. Nothing here is ever a float.
- [`formats/`](formats/): the tensor file, dense matrices, SMS,
  DIMACS and SMT-LIB, with [`interchange/`](formats/interchange/)
